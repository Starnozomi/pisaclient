...
public class WeatherViewPagerItemView extends LinearLayout implements OnClickListener, OnScrollListener {
	//主界面和背景view
	...
	public WeatherViewPagerItemView(Context context, String city) {
		super(context);
		this.mCity = city;
		...
		mCityDao =mContext.mCityDao;
		mWeatherDao = new WeatherDao();
		weeks = DateUtiles.getWeeks();
		days = DateUtiles.getDays();
//		mWeather = ExitApplication.getInstance().getWeather(mCity);
//		mTemperatureList = ExitApplication.getInstance().mTemperatureMap.get(mCity);
		//从本地数据库获取数据
		//mWeather = mCityDao.queryWeather(mCity);
		//mTemperatureList = mCityDao.getAllTemperatures(mCity);
		// if (mCity.equals("自动定位")) {
		// isLocation = true;
		// }
		// if (isLocation) {
		// mContext.setLocationCityName(mLocalArea);
		// mWeather = ExitApplication.getInstance().getWeather(mLocalArea);
		// mTemperatureList = ExitApplication.getInstance().mTemperatureMap.get(mLocalArea);
		// if (mLocalArea.equals("重庆市")) {
		// mWeather = ExitApplication.getInstance().getWeather("沙坪坝区");
		// mTemperatureList = ExitApplication.getInstance().mTemperatureMap.get("沙坪坝区");
		// }
		// }
		/*if (mCity.equals("重庆市")) {
//			mWeather = ExitApplication.getInstance().getWeather("沙坪坝区");
//			mTemperatureList = ExitApplication.getInstance().mTemperatureMap.get("沙坪坝区");
			mWeather = mCityDao.queryWeather("沙坪坝区");
			mTemperatureList = mCityDao.getAllTemperatures("沙坪坝区");
		}*/
	...
	private void setViews() {
		...
		mMainView = mInflater.inflate(R.layout.weather_main, null);
		mPager = mInflater.inflate(R.layout.weather_pager, null);
		
		mPull_down_view = (ImageView) mPager.findViewById(R.id.wt_pull_down_flag);
		mMyScrollView = (MyScrollView) mPager.findViewById(R.id.sv);
		mMyScrollView.setOnScrollListener(WeatherViewPagerItemView.this);
		mLl_content = (LinearLayout) mPager.findViewById(R.id.ll_content);
		
		mwt_current = mInflater.inflate(R.layout.weather_current, null);
//		mLl_remind_view = mInflater.inflate(R.layout.weather_remind, null);
...
//		mtv_wt_range = (TextView) mPager.findViewById(R.id.tv_wt_range);
	...
	private View createTransparentView() {
		...
		mwt_current = mInflater.inflate(R.layout.weather_current, null);
		measureView(mwt_current);
		mTransparentHeight = mContext.contentHeight - mwt_current.getMeasuredHeight();
		mContext.mTrans = mTransparentHeight;
...
//			mtv_wt_range.setText(weather.MinTemperature + "~" + weather.MaxTemperature + "℃");
				...
				if ((mWarningList != null) && (mWarningList.size() > 0)) {
					// isHaveWaring = mCityDao.isHaveWarnings(mCity);
					// miv_warming.setVisibility(isHaveWaring?View.VISIBLE:View.GONE);
				}
			} catch (Exception e) {
				e.printStackTrace();
	...
	public void onClick(View v) {
		// mContext.mBinder.setNoWarningRemind();
		switch (v.getId()) {
	...
	public void onScroll(int scrollY) {
		...
		int y = mTransparentHeight -mContext.contentHeight/2;
		
		if (scrollY<y) {
			mPull_up_view.setVisibility(View.VISIBLE);
			mPull_down_view.setVisibility(View.INVISIBLE);
		}else {
			mPull_up_view.setVisibility(View.INVISIBLE);
			mPull_down_view.setVisibility(View.VISIBLE);
		}
		
//		if (scrollY == 0) {
//			mPull_up_view.setVisibility(View.VISIBLE);
//			mPull_down_view.setVisibility(View.INVISIBLE);
////			mwt_current.findViewById(R.id.wt_pull_up_flag).setVisibility(View.VISIBLE);
//		} else if ((scrollY + mMyScrollView.getHeight()) == mMyScrollView.getChildAt(0).getMeasuredHeight()) {
//			mPull_up_view.setVisibility(View.INVISIBLE);
//			mPull_down_view.setVisibility(View.VISIBLE);
////			mwt_current.findViewById(R.id.wt_pull_up_flag).setVisibility(View.INVISIBLE);
//		} else {
//			mPull_up_view.setVisibility(View.INVISIBLE);
//			mPull_down_view.setVisibility(View.INVISIBLE);
////			mwt_current.findViewById(R.id.wt_pull_up_flag).setVisibility(View.INVISIBLE);
//		}
		...
		mContext.setback(scrollY,mTransparentHeight);
		
		// if (scrollY>mContext.contentHeight/2) {
		// mPull_up_view.setVisibility(View.INVISIBLE);
		// mPull_down_view.setVisibility(View.VISIBLE);
		// }
		// else {
	...
	public void smoothScrollTo(int x,int y){
		mMyScrollView.smoothScrollTo(x, y);
	}
