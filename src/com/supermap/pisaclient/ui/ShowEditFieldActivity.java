...
public class ShowEditFieldActivity extends Activity {
	...
	private void updateField() {
		...
		if (fieldkey.toLowerCase().contains("sm")) {
			CommonUtil.showToask(ShowEditFieldActivity.this, "系统字段" + fieldkey + "不能被修改！");
			return;
		}
		...
		.setMessage("请输入修改的内容").setView(layout).setPositiveButton("确定", new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface dialoginterface, int i) {
				...
				if (fieldValue == null || fieldValue.equals("")) {
					CommonUtil.showToask(getApplicationContext(), "请输入要修改的内容");
				} else {
					...
					CommonUtil.showToask(ShowEditFieldActivity.this, "字段" + fieldKey + "值修改为:" + fieldValue + "");
				}
			}
		}).setNegativeButton("取消", new DialogInterface.OnClickListener() { /* 设置跳出窗口的返回事件 */
			public void onClick(DialogInterface dialoginterface, int i) {
				CommonUtil.showToask(ShowEditFieldActivity.this, "取消了修改数据");
			}
		}).show();
	}
	...
	private void updateFields() {
		...
		if (feature == null) {
			CommonUtil.showToask(ShowEditFieldActivity.this, "农田为空，无法更新字段！");
			return;
		}
		...
		if (delResult != null && delResult.succeed) {
			CommonUtil.showToask(ShowEditFieldActivity.this, "更新字段成功！");
		} else {
			CommonUtil.showToask(ShowEditFieldActivity.this, "更新字段失败！");
		}
		this.onBackPressed();
	}
